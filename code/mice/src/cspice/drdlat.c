...
/* Subroutine */ int drdlat_(doublereal *r__, doublereal *lon, doublereal *
...
/*     Construct the matrix directly. */
    ...
    jacobi[0] = cos(*lon) * cos(*lat);
    jacobi[1] = sin(*lon) * cos(*lat);
    jacobi[2] = sin(*lat);
    jacobi[3] = -(*r__) * sin(*lon) * cos(*lat);
    jacobi[4] = *r__ * cos(*lon) * cos(*lat);
    jacobi[5] = 0.;
    jacobi[6] = -(*r__) * cos(*lon) * sin(*lat);
    jacobi[7] = -(*r__) * sin(*lon) * sin(*lat);
